...
% Chuan
%%
CID_len = length(CID_temp(CID_temp~=1));
...
for i = 1:CID_len
    sublist = getSubvector(Clist_temp, CID_temp,i);
    ...
    pi_1 = sum(cov_related_edges > r)/(nchoosek(CID_temp(i),2));
    pi_0 = sum(non_cov_related_edges>r)/(edgeLen - nchoosek(CID_temp(1),2));
